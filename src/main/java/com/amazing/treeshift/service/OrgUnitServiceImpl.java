...
public class OrgUnitServiceImpl implements OrgUnitService {
	...
	@Override
	@Transactional
	public Long persistTree(TreeOrgUnit tree) {
		// In a more normal application, we would use node IDs auto-generated by the database, and node.id
		// would be updated after the call to repository.save() — however, the main information-bearing piece
		// of data per node is the id, thus for an import to make any sense we need to be able to manually
		// specify the id instead. In other words, node.id should be equal to treeNode.id, and we don't really
		// need to pass on parentId.
		return persistTreeRecurse(tree, tree.getParentId());
	}

	private Long persistTreeRecurse(TreeOrgUnit treeNode, Long parentId) {
		var node = repository.save(new OrgUnit(treeNode.getId(), treeNode.getRootId(), treeNode.getHeight(), parentId));

		for (TreeOrgUnit child : treeNode.getChildren()) {
			persistTreeRecurse(child, node.getId());
		}
